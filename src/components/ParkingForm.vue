...
const currency = helpers.regex('numeric', /(\d{1,3}[.](\d{3}[.])*\d{3}|\d+)([,]\d{1,2})?$/);
...
export default {
    ...
    methods: {
        ...
        formatPrice () {
            ...
            this.parking.price = this.parking.price.toString()
                .replace(/\D/g, '')
                .replace(/\B(?=(\d{3})+(?!\d))/g, '.');
        },
        reformatPrice () {
